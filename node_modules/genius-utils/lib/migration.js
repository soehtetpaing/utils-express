...
export async function migrateVersion(databaseInfo = {}, migrateInfo = {}) {
    ...
    try {
        ...
        if (databaseType === 'mssql') {
            ...
            if (rows.recordset.length)  migratedVersion = rows.recordset[0].t1;

        } else if (databaseType === 'postgre' || databaseType === 'pg') {
            const query = 'SELECT t1 FROM vac001 ORDER BY autokey DESC LIMIT 1';
            const data = await pool.query(query);
            if (data.rows.length) migratedVersion = data.rows[0].t1;   

        } else if (databaseType === 'mysql') {
            const query = 'SELECT t1 FROM vac001 ORDER BY autokey DESC LIMIT 1';
            const [rows] = await pool.query(query);
            if (rows.length) migratedVersion = rows[0].t1;

        }

        let compare = compareVersions(version, migratedVersion, depth);
        if (compare <= 0) {
            // logging
            result = {
                status: 201,
        ...
        let ver = migratedVersion.split('.').map(Number);
        while (compare > 0) {
            ...
            const QUERIES = TEXT.split(/;\s*(?=\n|$)/i).map(q => q.trim()).filter(q => q.length);
            console.log(QUERIES);
...
function compareVersions(version = '', migratedVersion = '', depth = 3) {
    const v1 = version.split('.').map(Number);
    const v2 = migratedVersion.split('.').map(Number);
